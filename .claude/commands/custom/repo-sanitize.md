---
name: repo-sanitize
description: Whole-repo sanitation & validation — runs format, lint, stylelint, knip, secretlint, a type-check, the test suite, and the production build one by one, showing each step's output.
...
6. `pnpm run typecheck` — type check (`tsc -b` over the project references; `vite`/`vite-react-ssg build` does NOT run `tsc`, so this is the compile gate)
7. `pnpm test` — Vitest suite (includes the jest-axe accessibility checks)
8. `pnpm run build` — production build (`tsc -b && vite-react-ssg build` — client + server bundles, then prerender of `dist/index.html`)

Notes:

- Steps 1–3 modify files. After they run, mention if anything was changed.
- Step 6 exits non-zero (code `2`) on type errors — treat that as a failure and stop.
- These are long-running; do not background them — run in the foreground so I see live output.

## Final report (always produce this)

After the run ends — whether all steps passed or a step failed — print a final status table with **one row per step (all 8, always)**. Columns:
...
- Below the table:
  - If all 8 passed, add the one-line summary: ✅ repo sanitation & validation passed — and the total wall-clock runtime.
